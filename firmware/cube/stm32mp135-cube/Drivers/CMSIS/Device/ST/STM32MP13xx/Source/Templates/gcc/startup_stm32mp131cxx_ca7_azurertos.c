/**
  ******************************************************************************
  * @file    startup_stm32mp131cxx_ca7_azurertos.c
...
void Reset_Handler (void) __attribute__ ((naked, target("arm"))); /* The reset handler is always in ARM mode */
...
void __attribute__ ((weak)) __tx_irq_handler(void) {}
#endif
...
void Vectors(void) {
  ...
  "LDR    PC, =__tx_irq_handler                     \n"  /* Call ThreadX interrupt handler*/
...
#if !defined(ARM_MODE)
  /* Needed so that the assembler switch back to thumb mode.
     Otherwise the following code will stay in ARM mode (bug ?). */
  ".code 16                                        \n"
#endif
